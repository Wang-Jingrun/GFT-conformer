...
def load_wav(path, wav_file):
    ...
    # soundfile.write('output.wav', changed_wav, sr)
    return wav
...
def add_noise(wav, noise, snr):
    ...
    add_nosie = noise / (norm_noise + 1e-7) * norm(wav) / (10.0 ** (0.05 * snr)) if norm_noise > 0 else 0
...
class VCTKEval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        noisy_wav = load_wav(os.path.join(self.dataset_path, 'noisy_testset_wav'),
                             self.test_files[index] + '.wav')
        return torch.from_numpy(noisy_wav), torch.from_numpy(clean_wav), self.test_files[index]
